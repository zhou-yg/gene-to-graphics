...
exports.categoryGraphics = categoryGraphics;
...
function categoryGraphics(graphicsList) {
  var r = {};
  const categoryMap = {
    other: '其它',
    eye: '眼',
...
exports.default = trunkToPixi;

var _G = require('./G');

function trunkToPixi(data) {
  const graphics = [].concat(data).map(arr => {
    return arr.map(g => {
      ...
      switch (g.type) {
        case _G.Rect.name:
          return new _G.Rect(superConfig);
        case _G.Circle.name:
          return new _G.Circle(superConfig);
        case _G.Polygon.name:
          return new _G.Polygon(superConfig);
      }
    });
  });
  ...
  graphics.forEach(arr => {
    arr.forEach(obj => {
      var _obj$getAttrs = obj.getAttrs();

      let fill = _obj$getAttrs.fill;
      ...
      switch (obj.constructor) {
        case _G.Rect:
          (() => {
            var _obj$getAttrs2 = obj.getAttrs();

            let x = _obj$getAttrs2.x,
                y = _obj$getAttrs2.y,
                w = _obj$getAttrs2.w,
                h = _obj$getAttrs2.h;
        ...
        case _G.Circle:
          (() => {
            var _obj$getAttrs3 = obj.getAttrs();

            let x = _obj$getAttrs3.x,
                y = _obj$getAttrs3.y,
                r = _obj$getAttrs3.r;
...
//# sourceMappingURL=/trunkToPixi.map